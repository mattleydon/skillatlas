...
import { getForumCategory, getForumThread, initialForumThreads } from "../forum-data";
...
export default async function ForumThreadPage({
  ...
  return (
    <main className="forum-detail-shell relative min-h-screen overflow-hidden bg-[#F8FAFC] text-[#111827] transition-colors duration-300">
      ...
      <section className="relative z-10 mx-auto w-full max-w-5xl px-4 pb-16 pt-[108px] sm:px-6 sm:pt-[116px] lg:px-8 lg:pt-[145px]">
        <Link
          href={ROUTES.forum}
        ...
        {thread ? (
          <article className="overflow-hidden rounded-3xl border border-[#ff2fa8]/45 bg-white/92 shadow-sm backdrop-blur">
            <div className="border-b border-[#ff2fa8]/20 p-5 sm:p-8">
              ...
              <h1 className="max-w-4xl break-words text-2xl font-black tracking-tight sm:text-4xl">{thread.title}</h1>
            ...
            <div className="p-5 sm:p-8">
              <p className="break-words text-base font-semibold leading-8 text-gray-700">{thread.body}</p>
          ...
          <section className="rounded-3xl border border-[#ff2fa8]/45 bg-white/92 p-6 shadow-sm backdrop-blur sm:p-8">
            <p className="text-xs font-black uppercase tracking-[0.24em] text-[#19d3cf]">Local thread preview</p>
            <h1 className="mt-3 break-words text-2xl font-black tracking-tight sm:text-3xl">{fallbackTitle || "New Forum Thread"}</h1>
        ...
        <section className="mt-6 rounded-3xl border border-[#19d3cf]/35 bg-white/88 p-5 shadow-sm backdrop-blur">
          <p className="text-[11px] font-black uppercase tracking-[0.22em] text-[#ff2fa8]">Continue exploring</p>
          <p className="mt-2 text-sm font-semibold leading-relaxed text-gray-600">
            Return to the Forum to browse categories, search discussions, or start another local thread.
          </p>
          <Link
            href={ROUTES.forum}
            className="mt-4 inline-flex rounded-full bg-[#19d3cf] px-5 py-2.5 text-sm font-black text-white shadow-lg shadow-[#19d3cf]/20 transition hover:-translate-y-0.5"
          >
            Browse Forum
          </Link>
        </section>
