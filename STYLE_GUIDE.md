...
- Preserve the original PNG symbol and its geometry. An isolated CSS-mask stack uses that PNG's alpha silhouette. The lower `::before` layer retains approved brightness/depth: 85% opacity, light `saturate(1.25) brightness(1.1) contrast(1.12)`, dark `saturate(1.04) brightness(1.11)`. The top `::after` layer takes the exact unfiltered `--sa-brand-gradient` used by ATLAS and applies `mix-blend-mode: color`, preserving lower-layer luminosity while unifying hue/saturation. Do not independently tint the colour layer. The image beneath remains the fallback, with light `saturate(1.05) brightness(1.08) contrast(1.06)` and dark `saturate(0.94) brightness(1.05) contrast(1.03)`. Both mask layers are hidden in forced-colour mode. The wordmark is unchanged; no blur, cropping, shadows or new raster artwork.
